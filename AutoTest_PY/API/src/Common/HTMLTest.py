...
class HTML:
    def __init__(self):
        ...
        # 报告位置
        self.filename = os.path.abspath(os.path.join(os.getcwd(), ".\\..\\result")) + "\\report\\" + self.now + "report.html"

    def htmlTest(self,file,title,description):
        # 使用unittest.TestLoader().discover方法，运行以case.py结尾的所有py文件
        my_discover = unittest.TestLoader().discover(self.dir_test, file)
...
if __name__=="__main__":
    ...
    ht=HTML()
    ht.htmlTest("*.py","XXX项目接口自动化测试报告","各接口测试用例执行情况")
